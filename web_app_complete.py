...
if 'momrange' not in st.session_state:
    st.session_state['momrange'] = (0,len(data)-1) 
...
if col3.button('Next Channel'):
    if st.session_state.channel >= len(data):
        st.write('Reached last channel!')
    else:
        st.session_state.channel += 1
        st.session_state.oldchannel = copy.copy(st.session_state.channel)

st.session_state.channel = st.slider('Choose Channel',
    min_value=0, max_value=len(data)-1, value=st.session_state.oldchannel, step=1)
